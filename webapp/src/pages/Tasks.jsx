...
`)
  ...
  // Listen for job logs and status updates
  useEffect(() => {
    if (!socket) return

    const handleJobLog = (data) => {
      console.log('[Tasks] Received job_log event:', data)
      // Accept logs for current job or if no current job (first log)
      const jobMatches = !currentJob || !currentJob.job_id || data.job_id === currentJob.job_id
      if (data.job_id && jobMatches) {
        // Handle both single log message and full logs array
        if (data.logs && Array.isArray(data.logs)) {
          // Full logs array received - replace all logs
          setLogs(data.logs)
        } else if (data.log || data.message) {
          // Single log message - append to existing logs
          setLogs(prev => {
            const newLog = data.log || data.message
            // Avoid duplicates
    ...
    const handleJobStatus = (data) => {
      console.log('[Tasks] Received job_status event:', data)
      // Accept status for current job or if no current job (first status)
      const jobMatches = !currentJob || !currentJob.job_id || data.job_id === currentJob.job_id
      if (data.job_id && jobMatches) {
        setCurrentJob(prev => ({ 
          ...(prev || {}), 
        ...
        // Update logs if provided in status update
        if (data.logs && Array.isArray(data.logs)) {
          setLogs(data.logs)
        }
        ...
        if (data.status === 'completed' || data.status === 'failed') {
          setLogs(prev => {
            ...
            if (prev.length > 0 && prev[prev.length - 1] === finalMsg) {
              return prev
            }
  ...
  // Poll for job status if job is running (fallback if Socket.IO events fail)
  useEffect(() => {
    if (!currentJob || !currentJob.job_id) return
    if (currentJob.status === 'completed' || currentJob.status === 'failed') {
      // Still poll once more to get final logs/output_files
      const fetchFinal = async () => {
        try {
          const serverURL = getServerURL()
          const response = await fetch(`${serverURL}/status/${currentJob.job_id}`)
          if (response.ok) {
          ...
          console.error('[Tasks] Final status fetch error:', error)
        }
      }
      fetchFinal()
      return
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          const hasNewLogs = data.logs && Array.isArray(data.logs) && data.logs.length > (logs.length || 0)
          
          // Always update if status changed or if we have new logs
          if (statusChanged || hasNewLogs) {
            console.log('[Tasks] Polling: Job updated - status:', data.status, 'logs:', data.logs?.length || 0)
            setCurrentJob(prev => ({
              ...prev,
              ...data,
              output_files: data.output_files || prev?.output_files || [],
  ...
  }, [currentJob?.job_id, currentJob?.status, logs.length])
