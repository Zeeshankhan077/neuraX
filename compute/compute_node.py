...
Features:
    - Connects to Render-hosted server (https://neurax-1.onrender.com)
...
class NeuraXComputeNode:
    ...
    def __init__(self, signaling_url: str = None):
        ...
        """
        # Step 1: Set signaling URL (default to Render production server)
        if not signaling_url:
            signaling_url = os.getenv('SIGNALING_SERVER_URL', 'https://neurax-1.onrender.com')
...
async def main():
    ...
    parser.add_argument(
        '--signaling-url',
        default=None,
        help='Signaling server URL (defaults to https://neurax-1.onrender.com or SIGNALING_SERVER_URL env var)'
    )
...
# Notes:
# - Compute node connects to Render production server by default
