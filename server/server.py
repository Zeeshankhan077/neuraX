...
def execute_job_async(job_id):
    ...
    try:
        # Check if job still exists (might have been cleared)
        if job_id not in jobs:
            logger.error(f"Job {job_id} not found in jobs dict - may have been cleared")
            return
        ...
        try:
            ...
            logger.error(f"Execution error for job {job_id}: {exec_error}")
            add_log(job_id, f"[ERROR] Execution failed: {str(exec_error)}")
            result = {
                'exit_code': -1,
                'stdout': '',
...
def execute_blender(job_id, file_path, args):
    ...
    # Re-check Docker availability (in case Docker was started after server)
    global docker_available
    if not docker_available:
        add_log(job_id, "[BLENDER] Docker not detected at startup, re-checking...")
        docker_available = check_docker_available()
        if docker_available:
            add_log(job_id, "[BLENDER] Docker is now available!")
        else:
            add_log(job_id, "[BLENDER] ERROR: Docker not available - cannot execute Blender")
            add_log(job_id, "[BLENDER] Please ensure Docker Desktop is running and restart the server")
            return {
                'exit_code': -1,
                'stdout': '',
