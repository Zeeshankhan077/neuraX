...
def execute_job():
    ...
    try:
        ...
        eventlet.spawn(execute_job_async, job_id)
...
def handle_compute_node_registration(data):
    ...
    try:
        ...
        logger.info(f"Compute node registered: {node_specs['device']}")
        
        # Broadcast to all clients
        socketio.emit('compute_node_registered', node_specs, broadcast=True)
    ...
    try:
        ...
        try:
            ...
            docker_cmd = [
                ...
                '--read-only',
                '-v', f'{temp_file}:/tmp/task.py:ro',
                '-v', '/tmp:/tmp:rw',
                'python:3.10',
                'python', '/tmp/task.py'
            ]
            
            add_log(job_id, "[EXECUTE] Starting Python execution in sandbox...")
